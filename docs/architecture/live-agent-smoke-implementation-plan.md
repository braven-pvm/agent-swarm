...
- requirement coverage/status is derived from accepted evidence plus obligation state, not worker claims alone [10C-2A implements a derived ledger]
- full-product final acceptance is blocked unless indexed FR/AC coverage is complete [10C-1A/10C-1C implemented]
- `human_input_required` and `human_verification_required` are distinct statuses in events, coverage, reports, and UI API payloads [10C-2B implemented for verifier output and coverage human paths]
- human verification packet artifacts can be generated for refs that need human acceptance [10C-2B implemented]
- human verification results can be recorded and move refs from awaiting to accepted/failed/repairing state [10C-2C implemented]
- parent FR rollups can explain direct FR status vs child AC completion [10C-2A implemented for derived coverage/ledger state]
...
- `/api/coverage` exposes obligation status, verification mode, responsible party, criteria count, evidence state, human path, and rollup reason additively [10C-1 implements obligation status/mode/responsible party/criteria/expected outcome; 10C-2A adds derived ledger status, human path, direct status, parent/child refs, and rollup reason; 10C-2B adds human packet links]
...
7. Add human verification packet generation and statuses. [10C-2B implemented for packet generation and awaiting-human status; 10C-2C implemented result recording/sign-off]
