...
Next slice: Phase 9 clean real-run rebaseline, followed by Phase 10A operational requirements coverage.

Read `docs/architecture/live-agent-smoke-implementation-plan.md` before editing. Phase 1 through Phase 8C-19 plus probe-isolation and response-shape hardening are implemented or attempted as documented. Phase 9 reruns the real full-product smoke and inspects final active escalations, product workflow probe stability, probe isolation metadata, coverage state, and reset-first behavior with UI/product processes. Phase 10A then hardens requirements coverage so every FR/AC can explain status, owner, evidence, blockers, dependency state, next action, and last changed time. The full-product destination remains `docs/requirements/live-smoke-invoice-dashboard-product-spec.md`: after reset and run, the ultimate smoke should produce a small real invoice dashboard a human can open, or exact blockers explaining why not.
